...
(function () {
  ...
  function rpc(fn, args, asUser) {
    return fetch(URL + '/rest/v1/rpc/' + fn, {
      method: 'POST', headers: headers(asUser !== false), body: JSON.stringify(args || {})
    }).then(function (r) {
      if (!r.ok) return r.json().then(function (j) { return { ok: false, error: j }; }).catch(function () { return { ok: false }; });
